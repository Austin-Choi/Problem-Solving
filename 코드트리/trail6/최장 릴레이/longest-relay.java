...
public class Main {
    ...
    public static void main(String[] args) throws IOException{
        ...
        int[][][] dp = new int[1<<N][N][21];
        for(int m = 0; m<(1<<N); m++){
            for(int i = 0; i<N; i++){
                Arrays.fill(dp[m][i], -1);
            }
        }
        dp[1<<0][0][0] = 1;
            ...
            for(int last = 0; last<N; last++){
                ...
                for(int lc = 0; lc<=20; lc++){
                    ...
                    for(int next = 0; next < N; next++){
                        if((m & (1<<next)) != 0)
                            continue;
                        int nc = cost[last][next];
                        if(nc <= lc)
                            continue;
                        int nm = m | (1<<next);
                        dp[nm][next][nc] = Math.max(dp[nm][next][nc], dp[m][last][lc] + 1);
                        ans = Math.max(ans, dp[nm][next][nc]);
                        // bitcount 비용이 너무 큼
                        // 그냥 dp에 갯수 저장하는 쪽으로 변경
